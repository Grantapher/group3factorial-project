...
public class ParkManagerUI {
    ...
    public void menuOptions() {
        ...
        if (!"q".equals(choice) && choice.equals(VIEWJOBS.toString()) || choice.equals(CREATEJOB.toString()) ||
                        choice.equals(ADDPARK.toString())){ // ensure user enters 1 or 2
    ...
    private void option1(final Scanner theScan) {
        ...
        if (theScan.next().toLowerCase().equals("y"))
            System.out.print("Please enter the number of the job: ");
            jobIndex = theScan.nextInt();
            
        if (!jobs.get(jobIndex - 1).getVolunteers().isEmpty()) { //job has volunteers           
            List<Volunteer> volList = myUser.getVolunteers(jobs.get(jobIndex - 1));
            System.out.println("Volunteers to " + jobs.get(jobIndex - 1).getDescription() + ":\n");
    ...
    /**
     * Creates a new job and attempts to add a job. User will be informed
     * if attempt fails.
     */
    private void option2(final Scanner theScan) {
        ...
        try {// try to add job
            final boolean submitCheck =  myUser.submit(Calendar.getInstance(), title, parkName, location,
                    LocalDate.parse(startDate), LocalDate.parse(endDate), light, medium, heavy, description); 
