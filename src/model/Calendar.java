...
public class Calendar {
	...
	private Map<LocalDate, ArrayList<Job>> dateToListOfJobs;
	
	/**
	 * Constructor. 
	 * This will eventually read in the persistent data from a File, and
	 * store it back into this Calendar.
...
//		fileReader.close();
	 ...
	 * @throws FileNotFoundException if the persistent Jobs.txt file is not found.
	 */
	public void addJob(Job job) {		
		if(!isFull() 
			&& !isFull(job.getStartDate()) 
			&& !isFull(job.getEndDate())
			&& isValidLength(job)
			&& isValidInterval(job)) {
				addJobToMap(job);
				//addJobToFile(job);
	 ...
	 * @return true if this Job is not in the past, and within the
	 * max number of days we want to keep track of.
	 */
	boolean isValidInterval(Job job) {
		LocalDate startDate = job.getStartDate();
	 ...
	 * @return true if this week contains the max number of Jobs it can
	 * handle for a week.
	 */
	boolean isFull(LocalDate date) {
		int jobsThisWeek = 0;
		LocalDate startDate = date.minusDays(HALF_WEEK);
		LocalDate endDate = date.plusDays(HALF_WEEK);
		...
		// based on date param
		for (LocalDate day = LocalDate.from(startDate); 
	...
	boolean isFull() {
		int totalJobs = 0;
		Set<LocalDate> allDaysInCalendar = dateToListOfJobs.keySet();
		ArrayList<Job> listOfJobsThisDay = null;
		
		for (LocalDate date : allDaysInCalendar) {
	...
	void addJobToMap(Job job) {
		boolean dateIsInCalendar = dateToListOfJobs.containsKey(job.getStartDate());
